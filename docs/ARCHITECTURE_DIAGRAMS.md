...
**Nothing below has been built.** Every AWS box is explicitly proposed —
see `docs/AWS_MIGRATION_PLAN.md` for the full reasoning behind each choice,
including the alternatives that were considered and why they weren't picked
...
flowchart TD
    ...
    AppRunner[PROPOSED: AWS App Runner<br/>runs the same FastAPI app,<br/>containerized]
    ...
    CW[PROPOSED: CloudWatch<br/>default App Runner logging]
    ...
    CF_prop -->|serves| S3
    User -->|HTTPS request for /api/*| AppRunner
    AppRunner --> Tools
    Tools -->|messages.create| Anthropic
    AppRunner -.->|reads secret| Params
    AppRunner -.->|sends logs| CW
    GitHub -.->|CI/CD deploy| S3
    GitHub -.->|CI/CD deploy| AppRunner

    style Anthropic fill:#e8f0ff
    style CF_prop fill:#d6ecff,stroke-dasharray: 5 5
    style S3 fill:#d6ecff,stroke-dasharray: 5 5
    style AppRunner fill:#d6ecff,stroke-dasharray: 5 5
...
shown so the absence is a decision, not an oversight.
