...
If the source resume already states its own total years-of-experience figure in words (e.g. "over 20+ years of experience"), defer to that stated figure - restate it as-is or in very close conservative language, but do not recalculate a different number. You have no reliable way to know how old the resume itself is or what "Present" meant at the time it was written, so calculating fresh from today's date can produce a number that contradicts what the candidate already said about their own career, which is exactly the kind of unsupported claim to avoid.
Only calculate a total years-of-experience figure yourself when the source resume does NOT already state one, and only when the chronology from clearly stated employment dates makes the calculation unambiguous. When you do calculate, use conservative, rounded-down phrasing such as "30+ years of experience" rather than a falsely precise number like "31.5 years," and never double-count overlapping positions (e.g. two roles held concurrently, or a stated date range that overlaps another) - calculate from the span of the career, not the sum of each job's individual duration.
