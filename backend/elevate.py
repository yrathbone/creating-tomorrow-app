...
4. PROFESSIONAL EXPERIENCE: rewrite each job's bullets with polished, accurate language, and fold each confirmed fact's bullet_text into the single most relevant existing job entry (matching by category, timing, or context) - never invent a new employer or role to hold it. If a confirmed fact doesn't clearly belong to one job more than another, add it to the most recent relevant role.
...
- "verify": a short reminder list of things the candidate should double check - always include employment dates, job titles, and contact information; also include any metrics and, if any confirmed facts were added, "newly confirmed experience" as an item.
