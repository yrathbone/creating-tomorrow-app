# Video 4: "Missing a Skill? Here's What Actually Works"
...
- Palette: cobalt blue `#0047AB` and gold `#F0AC3F` accents; use `--error` red (`#B3261E`) only for the warning flag in scene 1, nowhere else, so it doesn't read as alarming overall

| # | Time | Visual / Animaker scene direction | On-screen text | Narration (voiceover) |
|---|------|-----------------------------------|-----------------|------------------------|
| 1 | 0:00–0:14 | A resume document with a skill word being typed onto it, then a small red flag icon pops up next to it. | "Wait — do you actually have this?" | "When a job posting asks for a skill you don't have, the tempting move is to just... type the word in anyway. Don't. A resume that says you have a skill you don't is a problem waiting for your first day on the job." |
